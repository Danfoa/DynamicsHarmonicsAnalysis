...
class DynamicAE(LatentBaseModel):
    ...
    def __init__(self,
        ...
        self._state_obs_shape = None  # Shape of the state observable / input features
    ...
    def fit(
        ...
        self.lightning_model = LightningLatentModel(
            latent_model=self,
            optimizer_fn=optimizer_fn,
            optimizer_kwargs=optimizer_kwargs,
            )

        assert isinstance(self.lightning_module, lightning.LightningModule)
        ...
        if self.loss_weights is None:  # Automatically set the loss weights if not provided
            input_dim = np.prod(self._state_obs_shape)
            # Weight evenly a delta error in state space as a delta error in latent observable space
            obs_dim_state_dim_ratio = math.sqrt(self.latent_dim / input_dim)
    ...
    def compute_loss_and_metrics(self,
                                 state_context: TensorContextDataset,
                                 pred_state_context: TensorContextDataset,
                                 latent_obs_context: TensorContextDataset,
                                 pred_latent_obs_context: TensorContextDataset,
        ...
        rec_loss = MSE(state_context.lookback(lookback_len), pred_state_context.lookback(lookback_len))
        pred_loss = MSE(state_context.lookforward(lookback_len), pred_state_context.lookforward(lookback_len))
        ...
        if self.use_lstsq_for_evolution:
            ...
            lin_loss = MSE(latent_obs_context.data, pred_latent_obs_context.data)
    ...
    def training_step(self, train_batch, batch_idx):
        ...
        loss = alpha_rec * rec_loss + alpha_pred * pred_loss
        metrics = {"train/reconstruction_loss": rec_loss.item(), "train/prediction_loss": pred_loss.item()}
    ...
    def predict(
        ...
        """
        Predicts the state or, if the system is stochastic, its expected value :math:`\mathbb{E}[X_t | X_0 = X]`
        ...
        assert tuple(data.shape[2:]) == self._state_observables_shape
    ...
    def modes(self, data: TensorContextDataset, predict_observables: bool = True):
        raise NotImplementedError()

    def eig(self,
            eval_left_on: Optional[TensorContextDataset] = None,
            eval_right_on: Optional[TensorContextDataset] = None) -> Union[
        np.ndarray,
    ...
    def initialize_evolution_operator(self, init_mode: str):
        ...
        if self.use_lstsq_for_evolution:
            ...
            if init_mode == "stable":
                self.linear_dynamics.weight.data = torch.eye(self.state_dim)
    ...
    def dry_run(self, batch: TensorContextDataset):
        ...
        # TODO: Lightning has already sanity check runs before each train starts. Is this necessary?
        Z = encode_contexts(batch, self.encoder)
        if self.use_lstsq_for_evolution:
            evolution_operator = self._lstsq_evolution(Z)
        else:
            evolution_operator = self.evolution_operator
        Z_evolved = evolve_contexts(Z, self.lookback_len, evolution_operator)
        if Z.shape != Z_evolved.shape:
            raise ShapeError(f"Encoded latent context shape {Z.shape} different from evolved one {Z_evolved.shape}")

        X_evol = decode_contexts(Z_evolved, self.decoder)
        if batch.shape != X_evol.shape:
            raise ShapeError(f"Decoded context shape {X_evol.shape} different from input context shape {batch.shape}")
    ...
    def _check_dataloaders_and_shapes(self, datamodule, train_dataloaders, val_dataloaders):
        ...
        if train_dataloaders is None:
            ...
            for batch in datamodule.train_dataloader():
                ...
                with torch.no_grad():
                    self.dry_run(batch)
                    self._state_obs_shape = tuple(batch.shape[2:])
                break
        else:
    ...
    def _preprocess_for_eigfun(self, data: TensorContextDataset):
        ...
        contexts_for_inference = data_ondevice.slice(slice(self.lookback_len - 1, self.lookback_len))
    ...
    def configure_optimizers(self):
        optimizer = self.optimizer_fn if self.optimizer_fn is not None else torch.optim.Adam
        kw = self.optimizer_kwargs | {"lr": self.optimizer_kwargs.get("lr", 1e-3)}
        return optimizer(self.parameters(), **kw)
