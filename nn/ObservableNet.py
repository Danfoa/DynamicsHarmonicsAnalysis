...
class ObservableNet(torch.nn.Module):
    ...
    def __init__(
        ...
        if self.equivariant:
            ...
            assert obs_type is not None, "obs state Field type must be provided when using equivariant encoder"
            self.obs_H = escnn.nn.Linear(in_type=self.encoder.out_type, out_type=obs_type, bias=True)
            if explicit_transfer_op:
                ...
                self.obs_H_prime = escnn.nn.Linear(in_type=self.encoder.out_type, out_type=obs_type, bias=True)
        else:
            assert obs_dim is not None, "obs state dimension must be provided when using non-equivariant encoder"
            self.obs_H = torch.nn.Linear(in_features=self.encoder.out_dim, out_features=obs_dim, bias=True)
            if explicit_transfer_op:
