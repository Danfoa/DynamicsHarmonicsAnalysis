...
def plotNdimTraj(traj=None, traj_des=None, fig: Figure = None, show=True, figTitle="", eigvals=None,
    ...
    if traj_des is not None:
        assert traj_des.shape == traj.shape, (f"Real and desired state trajs differ in size {traj_des.shape} != "
                                              f"{traj.shape}")
...
def plot_system_3D(A, trajectories, constraint_matrix=None, constraint_offset=None, fig=None, initial_call=False,
                   flow_field_colorscale='Blues', traj_colorscale='Viridis',
                   init_state_color='red', initial_point_radius=3):
    trajs = np.asarray(trajectories)
...
def plot_trajectories(trajs, secondary_trajs=None, fig=None, colorscale='Prism',
                      ...
                      main_legend_label=None, secondary_legend_label=None, shade_area=False,
    ...
    for dim in range(state_dimension):
        ...
        # Add a title for this subplot, but only if it hasn't been added yet
        if dim_names[dim]:
...
def plot_two_panel_trajectories(state_trajs, obs_state_trajs, pred_obs_state_trajs=None,
    ...
    obs_state_dim_names = ["x_{:d}(w)".format(i+1) for i in range(state_dimension)]
